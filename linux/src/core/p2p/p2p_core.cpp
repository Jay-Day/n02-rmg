...
// Debug trace macro (no-op for now, can be enabled for debugging)
#define n02_TRACE()

struct p2p_chatstruct {
    int crframeno;
    bool local;
    char msg[251];
};

static oslist<p2p_chatstruct, P2P_CHAT_BUFFER_LEN> p2p_chat_cache;
...
static P2PCORESTAT P2PCORE;
static bool p2p_core_initialized = false;

// Time management
static uint32_t p2p_initial_time;
#define ADJUST_RATIO_T 1/30

static void p2p_InitializeTime() {
    p2p_initial_time = Timer::getTickCount();
}

static int p2p_GetTime() {
    return Timer::getTickCount() - p2p_initial_time;
}

static void p2p_AdjustTime(int TIME) {
    p2p_initial_time += ((p2p_initial_time - TIME) * ADJUST_RATIO_T);
}

static int p2p_PING_TIME;
...
bool p2p_disconnect() {
    ...
    if (P2PCORE.CONNECTED) {
        if (P2PCORE.status > 1) {
            p2p_core_debug("Can't quit while in game");
            return false;
        } else {
...
bool p2p_core_initialize(bool host, int port, const char* appname, const char* gamename, const char* username) {
    ...
    P2PCORE.HOST = host;
    P2PCORE.PORT = port;
    std::strncpy(P2PCORE.APP, appname, 128);
    std::strncpy(P2PCORE.USERNAME, username, 32);
    std::strncpy(P2PCORE.GAME, gamename, 128);
...
void p2p_print_core_status() {
    p2p_core_debug("P2PCORE{\n"
        ...
        P2PCORE.PORT,
        P2PCORE.HOST ? "irrelevant" : P2PCORE.IP,
        P2PCORE.APP,
        P2PCORE.GAME,
...
void p2p_send_chat(const char* xxx) {
    ...
    std::strncpy(ps->msg, xxx, std::min((size_t)251, std::strlen(xxx) + 1));
    ps->local = true;
...
static inline void p2p_handle_chat_instruction(p2p_instruction* ki) {
...
static bool p2p_WaitForPeerToLoadOrDie() {
    n02_TRACE();
    sockaddr_in saddr;
    ...
    while (!P2PCORE.PEERLOADED && P2PCORE.status == 1) {
        if (P2PCORE.connection->has_data() ||
            (k_socket::check_sockets(1, 0) && P2PCORE.connection->has_data())) {
            p2p_instruction ki;
...
static bool p2p_CalculatePingOrDie() {
    n02_TRACE();
#define p2p_PING_TIMES 4
    ...
    if (P2PCORE.HOST) {
        P2PCORE.ping = (p2p_GetTime() - p2p_PING_TIME) / p2p_PING_TIMES;
...
static bool p2p_SynChronizeClocksOrDie() {
    n02_TRACE();
    sockaddr_in saddr;
            ...
            case LOGN: {
                if (P2PCORE.HOST && !P2PCORE.CONNECTED) {
                    if (P2PCORE.ping == -1 && ki.inst.flags == LOGN_REQ) {
                        ...
                        p2p_core_debug("Connection Request from %s (%s).. Waiting for reconfirmation...",
                                       P2PCORE.PEERNAME, peerapp);
                        p2p_peer_info_callback(P2PCORE.PEERNAME, peerapp);

                        if (std::strcmp(peerapp, P2PCORE.APP) != 0) {
                    ...
                    if (P2PCORE.ping == 0 && ki.inst.flags == LOGN_RPOS) {
                        ...
                        p2p_send_chat("Using version: " P2P_VERSION " - Things may behave unexpectedly if different versions are used");
                    ...
                    if (P2PCORE.ping == 0 && ki.inst.flags == LOGN_RNEG) {
                        P2PCORE.ping = -1;
                        p2p_core_debug("Peer dropped connection (probably doesn't have the game)");
                    }
                } else {
                    if (!P2PCORE.HOST && !P2PCORE.CONNECTED) {
...
static inline bool ProcessGameStuff() {
    n02_TRACE();
    ...
    if (P2PCORE.status > 1) {
        ...
        if (P2PCORE.frameno++ >= P2PCORE.throughput) {
            ...
            while (P2PCORE.PEERDATA.pos < P2PCORE.DATALEN && P2PCORE.status > 1) {
                if (P2PCORE.connection->has_data() ||
                    (k_socket::check_sockets(0, 10) && P2PCORE.connection->has_data())) {
                    if (ProcessGameStuff()) {
                        return -1;
                    }
                } else {
